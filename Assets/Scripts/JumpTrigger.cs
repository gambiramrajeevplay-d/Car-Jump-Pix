...
{
    public enum JumpDirection { Straight, Left, Right }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        BoostTrigger boost = other.GetComponent<BoostTrigger>();
        if (boost == null) return;

        Vector3 dir = other.transform.forward;

        if (jumpDirection == JumpDirection.Left)
            dir = Quaternion.Euler(0f, -jumpAngle, 0f) * other.transform.forward;
